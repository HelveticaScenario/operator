...
fn static_range_output_exposes_virtual_range_ports() {
    ...
    assert!(
        (min - (-5.0)).abs() < 0.01,
        "static rangeMin should be -5, got {min}"
    );
    assert!(
        (max - 5.0).abs() < 0.01,
        "static rangeMax should be 5, got {max}"
    );
}
...
fn remap_dynamic_range_tracks_outMin_outMax() {
    ...
    assert!(
        (min - (-3.0)).abs() < 0.1,
        "remap rangeMin should be ~-3, got {min}"
    );
    assert!(
        (max - 7.0).abs() < 0.1,
        "remap rangeMax should be ~7, got {max}"
    );
}
...
fn wrap_dynamic_range_uses_min_max() {
    ...
    assert!(
        (min - 1.0).abs() < 0.01,
        "wrap rangeMin should be 1, got {min}"
    );
    assert!(
        (max - 4.0).abs() < 0.01,
        "wrap rangeMax should be 4, got {max}"
    );
}
...
fn wrap_dynamic_range_swaps_when_max_lt_min() {
    ...
    assert!(
        (min - 0.0).abs() < 0.01,
        "wrap rangeMin should be 0 (swapped), got {min}"
    );
    assert!(
        (max - 5.0).abs() < 0.01,
        "wrap rangeMax should be 5 (swapped), got {max}"
    );
}
...
fn spread_dynamic_range_uses_min_max() {
    ...
    assert!(
        (min - 2.0).abs() < 0.01,
        "spread rangeMin should be 2, got {min}"
    );
    assert!(
        (max - 8.0).abs() < 0.01,
        "spread rangeMax should be 8, got {max}"
    );
}
...
fn remap_dynamic_range_swaps_when_out_min_gt_out_max() {
    ...
    assert!(
        (min - (-5.0)).abs() < 0.1,
        "remap rangeMin should be -5 (ordered), got {min}"
    );
    assert!(
        (max - 5.0).abs() < 0.1,
        "remap rangeMax should be 5 (ordered), got {max}"
    );
}
...
fn scale_and_shift_dynamic_range_negative_gain() {
    ...
    assert!(
        (min - (-5.0)).abs() < 0.1,
        "rangeMin should be ~-5 (reordered), got {min}"
    );
    assert!(
        (max - 5.0).abs() < 0.1,
        "rangeMax should be ~5 (reordered), got {max}"
    );
}
...
fn clamp_dynamic_range_orders_inverted_bounds() {
    ...
    assert!(
        (min - (-2.0)).abs() < 0.1,
        "clamp rangeMin should be ~-2 (ordered), got {min}"
    );
    assert!(
        (max - 3.0).abs() < 0.1,
...
fn dynamic_range_is_read_at_the_consumer_sample_slot() {
    ...
    let patch =
        Patch::from_graph(&graph, SAMPLE_RATE, BS, &HashMap::new()).expect("from_graph failed");
