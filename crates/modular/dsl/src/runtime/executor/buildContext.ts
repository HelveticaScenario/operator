...
import { create$ott } from '../composites/$ott';
import { create$cross } from '../composites/$cross';
...
export function buildContext(
    ...
    const $ott = create$ott({ namespaceTree: userNamespaceTree, $mix });
    const $cross = create$cross(userNamespaceTree);
    ...
    const dslGlobals = {
        ...
        $buffer,
        $delay,
        // 3-band compressor composite
        $ott,
        // N-way crossfade via $track keyframes
        $cross,
        // WAV sample loading
        $wavs,
        // Built-in modules
